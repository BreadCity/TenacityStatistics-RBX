-- Taken straight from Orca - https://github.com/richie0866/orca/
-- Licensed under the MIT License
local PARAMS = {...}

local function getFlag(flag)
  for _, v in ipairs(PARAMS) do if v == flag then return true end end
  return false
end
...
local function writeInstanceTree(object, output)
  ...
  for _, child in ipairs(object:GetChildren()) do writeInstanceTree(child, output) end
end
...
local function main()
  ...
  if MINIFY then output = {minify(table.concat(output, '\n'))} end
